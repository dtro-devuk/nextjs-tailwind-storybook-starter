...
const I = styled.i`
  ${tw`mt-1 mr-2 text-center text-blue-600 hover:text-yellow-500`}
`;
...
const A = styled.a(({ disabled }) => [
  tw`inline-block w-auto h-10 text-blue-600 cursor-pointer hover:text-yellow-500`,
  tw`transition duration-500 ease-in-out transform hover:scale-x-105`,

  disabled && tw`cursor-not-allowed`,
]);

const LinkContainer = styled.link(({ disabled }) => [disabled && tw`cursor-not-allowed`]);
